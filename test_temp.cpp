...
void callback(char* topic, byte* payload, unsigned int length)
{
    ...
    message[length] = '\0';
    Serial.println();
    if(strcmp(topic, "iiot-dta/check") == 0 && strcmp(message, "yey") == 0)
    {
        readData();
        mqtt_client.publish("iiot-dta/check", "Dado salvo com sucesso!");
    }
}
...
void readData()
{
    ...
    {
        digitalWrite(PIN_LEDS[3],  1);
        digitalWrite(PIN_LEDS[2],  1);
        digitalWrite(PIN_LEDS[1],  0);
    }
